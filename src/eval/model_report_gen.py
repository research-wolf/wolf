...
def eval_model(args):
    ...
    if model_path is not None:
        ...
        if "vicuna" in model_base.lower() and model_path is not None:
            ...
            tokenizer.add_tokens([DEFAULT_IM_END_TOKEN, DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN])
    ...
    cdatas = []
    canswer_file_path = ""
    cdatas = json.load(open(canswer_file_path, "r"))

    # upload report
    questions = copy.deepcopy(cdatas)
    ...
    for qid, line in enumerate(tqdm(questions[start_idx:]), start=start_idx):
        patient_id, study_id, dicom_id = line['patient_id'], line['study_id'], line['dicom_id']
        image_file = os.path.join(args.image_root, line["image"])
        qs = DEFAULT_IMAGE_TOKEN # no question when stage2
        if mm_use_im_start_end:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model-path", type=str, default="lmsys/vicuna-7b")
    parser.add_argument("--model-base", type=str, default="lmsys/vicuna-7b")
